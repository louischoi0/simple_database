...
class buffer_cursor:
    """
    메모리(bytearray) 또는 파일(mmap) 위에서 동작하는 커서.

    메모리 모드 (기본값)
    --------------------
        bc = buffer_cursor()
        bc = buffer_cursor(buffer=bytearray(4096))

    파일 모드 (mmap)
    ----------------
        # 파일이 없으면 size 바이트로 새로 생성
        bc = buffer_cursor.from_file("data.bin", size=4096)

        # 이미 있는 파일 열기 (기존 내용 유지)
        bc = buffer_cursor.from_file("data.bin")

        # 읽기 전용
        bc = buffer_cursor.from_file("data.bin", writable=False)

    두 모드 모두 동일한 read_*/write_* API를 사용한다.
    파일 모드에서는 커서를 닫을 때 flush/close를 호출해야 한다.
    context manager를 권장한다:

        with buffer_cursor.from_file("data.bin", size=4096) as bc:
            bc.write_int64(42)
    """
    ...
    def from_file(
        ...
        """mmap 파일 백엔드로 buffer_cursor를 생성한다."""
    ...
    @property
    def buffer(self):
        """메모리 백엔드일 때만 raw bytearray를 반환한다."""
